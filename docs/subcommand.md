...
**重要**：部分补全函数（如 `_hx`）使用 `_arguments`，其底层 `comparguments` 是 C builtin，**完全绕过** function-level `compadd` hook，导致 `_FZF_BLE_POOL` 永远为空。这种情况下会自动降级到 `--help` 解析路径，不再静默退出。

### `--help` 解析（`_ble_parse_help` 状态机）

`_ble_parse_help` 是单次扫描状态机，先识别 section header 再按状态提取。
...
**效果**：`node bu<Tab>` → `word` 变 `"--bu"` → 前缀匹配 `--build-sea`、`--build-snapshot` 等。`_BLE_PFX` 仍为原 `prefix`（`"node "`），候选直接替换原始 word（`"bu"` 被替换为 `"--build-sea"`）。
